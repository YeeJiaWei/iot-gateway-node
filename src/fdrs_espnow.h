...
#ifdef ESPNOW_NEIGHBOR_1
uint8_t ESPNOW1[] = {MAC_PREFIX, ESPNOW_NEIGHBOR_1};
#endif
#ifdef ESPNOW_NEIGHBOR_2
    ...
    switch (interface) {
        case 0: {
            ...
            for (int i = 0; i < lenESPNOWG; i++) {
                if (j > espnow_size) {
                    j = 0;
                    esp_now_send(broadcast_mac, (uint8_t *)&thePacket,
                                 sizeof(thePacket));
                }
        ...
        case 1: {
            ...
            for (int i = 0; i < lenESPNOW1; i++) {
                if (j > espnow_size) {
                    j = 0;
                    esp_now_send(ESPNOW1, (uint8_t *)&thePacket,
                                 sizeof(thePacket));
                }
        ...
        case 2: {
            ...
            for (int i = 0; i < lenESPNOW2; i++) {
                if (j > espnow_size) {
                    j = 0;
                    esp_now_send(ESPNOW2, (uint8_t *)&thePacket,
                                 sizeof(thePacket));
                }
