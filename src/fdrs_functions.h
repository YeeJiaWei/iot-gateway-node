...
#else
// ASSERT("NO WiFi SSID defined! Please define in fdrs_globals.h (recommended)
// or in fdrs_sensor_config.h");
#endif  // WIFI_SSID
...
#else
// ASSERT("NO WiFi password defined! Please define in fdrs_globals.h
// (recommended) or in fdrs_sensor_config.h");
#endif  // WIFI_PASS
...
#else
// ASSERT("NO MQTT address defined! Please define in fdrs_globals.h
// (recommended) or in fdrs_sensor_config.h");
#endif  // MQTT_ADDR
...
#else
// ASSERT("NO MQTT user defined! Please define in fdrs_globals.h (recommended)
// or in fdrs_sensor_config.h");
#endif  // MQTT_USER
...
#else
// ASSERT("NO MQTT password defined! Please define in fdrs_globals.h
// (recommended) or in fdrs_sensor_config.h");
#endif  // MQTT_PASS
...
#else
// ASSERT("NO LORA-BAND defined! Please define in fdrs_globals.h (recommended)
// or in fdrs_sensor_config.h");
#endif  // LORA_BAND
...
#else
// ASSERT("NO LORA-SF defined! Please define in fdrs_globals.h (recommended) or
// in fdrs_sensor_config.h");
#endif  // LORA_SF
...
#else
// ASSERT("NO LORA-TXPWR defined! Please define in fdrs_globals.h (recommended)
// or in fdrs_sensor_config.h");
#endif  // LORA_TXPWR
...
#ifdef USE_LORA
uint16_t LoRa1 = ((mac_prefix[4] << 8) |
                  LORA_NEIGHBOR_1);  // Use 2 bytes for LoRa addressing instead
                                     // of previous 3 bytes
uint16_t LoRa2 = ((mac_prefix[4] << 8) | LORA_NEIGHBOR_2);
uint16_t loraGwAddress = ((selfAddress[4] << 8) |
                          selfAddress[5]);  // last 2 bytes of gateway address
uint16_t loraBroadcast = 0xFFFF;
unsigned long receivedLoRaMsg =
    0;  // Number of total LoRa packets destined for us and of valid size
...
// CRC16 from
// https://github.com/4-20ma/ModbusMaster/blob/3a05ff87677a9bdd8e027d6906dc05ca15ca8ade/src/util/crc16.h#L71
...
void sendLog() {
    ...
    for (int i = 0; i < ln; i++) {
        ...
        outgoingString =
            outgoingString + " " + stringCrc(outgoingString.c_str()) + "\r\n";
        if (logBufferPos + outgoingString.length() >=
            (sizeof(logBuffer) /
             sizeof(char)))  // if buffer would overflow, release first
        {
            releaseLogBuffer();
        }
        memcpy(&logBuffer[logBufferPos], outgoingString.c_str(),
               outgoingString.length());  // append line to buffer
...
void reconnect(int attempts) { reconnect(attempts, false); }
...
int getFDRSPeer(uint8_t* mac) {  // Returns the index of the array element that
    ...
    switch (theCmd.cmd) {
...
#endif
                esp_now_send(incMAC, (uint8_t*)&sys_packet,
                             sizeof(SystemPacket));
        ...
        case cmd_add:
            ...
            if (peer_num == -1) {  // if the device isn't registered
                ...
                memcpy(&peer_list[open_peer].mac, &incMAC,
                       6);  // save MAC to open spot
...
#endif
                SystemPacket sys_packet = {.cmd = cmd_add,
                                           .param = PEER_TIMEOUT};
                esp_now_send(incMAC, (uint8_t*)&sys_packet,
                             sizeof(SystemPacket));
